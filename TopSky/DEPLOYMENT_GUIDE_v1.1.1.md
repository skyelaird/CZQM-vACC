# TopSkyMaps.txt v1.1.1 - Deployment Guide

**Version:** 1.1.1  
**Date:** 2025-11-20  
**Previous Version:** 1.1.0 (had incorrect ACTIVE field usage)  
**File Size:** 2,010 lines  
...
### ✅ Critical Fix from v1.1.0
- **Corrected ACTIVE field usage** - Now uses Field 3 (OnlineIdList) instead of Field 1 (YourIdList)
- **Why:** Boundaries show delegation awareness (when positions ARE online) not egocentric view (when YOU are that position)

### Operational Logic Correction

**v1.1.0 (Wrong):**
```
ACTIVE:ID:HZT:*:*:*     // Show when YOU are HZT
```
This would only show Halifax Tower boundary when YOU log on as HZT.

**v1.1.1 (Correct):**
```
ACTIVE:ID:*:*:HZT:*     // Show when HZT is online
```
This shows Halifax Tower boundary when ANYONE logs on as HZT → center sees "that's delegated."

### Simplified Always-Visible

**Class G boundaries now use:**
```
ACTIVE:1                // Auto-activate on plugin load
```
Instead of `ACTIVE:ID:*:*:*:*`
